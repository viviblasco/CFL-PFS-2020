...
var GestorPacientes_1 = require("./GestorPacientes");
...
miGestor1 = new GestorPacientes_1["default"]('./infoPacientes.txt');
console.log(miGestor1);
console.log('');
console.log('Creando los registros sin archivo:');
miGestor2 = new GestorPacientes_1["default"]();
...
function ejecutar() {
    accion = menu();
    if (accion === 1) {
        var doctor = readlineSync.question("Doctor/a: ");
        var Dni = readlineSync.questionInt("Dni: ");
        var nombreYApellido = readlineSync.question("Nombre y Apellido: ");
        var edad = readlineSync.questionInt("edad: ");
        var telefono = readlineSync.questionInt("telefono: ");
        miGestor1.agregarPaciente(doctor, Dni, nombreYApellido, edad, telefono);
    }
    else if (accion === 2) {
        var doctor = readlineSync.question("Ingrese el doctor/a que lo atiende: ");
        miGestor1.verPacientePorDoctor(doctor);
    }
    else if (accion === 3) {
        var id = readlineSync.questionInt("Ingrese el id del paciente a borrar: ");
        miGestor1.borrarPaciente(id);
    }
    else if (accion === 4) {
        var id = readlineSync.questionInt("Ingrese el id del paciente a actualizar: ");
        var doctor = null;
        var Dni = null;
        var nombreYApellido = null;
        var edad = null;
        var telefono = null;
